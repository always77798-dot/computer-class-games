...
export default function MapRescueGame({ onBackToPortal }) {
  ...
  // ---------------- Text & Configs ----------------
  const levels = {
    intro: { title: "Google地圖探險", subtitle: "迷航救援大作戰", desc: "急徵地圖超能力者！小探險家在土城國小迷失了方向，只有你能帶領他突破困境！本次探險採用【連擊計分】機制，請盡情展現你的最強實力！哲民老師將在終點見證最強冒險者的誕生！", btnText: "開始探險！" },
    level1: { title: "任務一：定位與搜尋", task: "點擊上方「搜尋按鈕」找尋土城國小，然後精準點擊地圖上的「紅色標記」！" },
    level2: { title: "任務二：街景降落", task: "將「衣夾人」拖曳到馬路。轉動視角前進，瞄準並拍攝「校門口」！" },
    level3: { title: "任務三：路線規劃", task: "閱讀情境線索，並設定正確的「起點」、「終點」與「交通方式」！" },
    gameIntro: { title: "第二部分：趣味大挑戰", task: "即將進入三道充滿挑戰的街機關卡！連擊越高分數越多，出發！(若單一關卡失敗3次將降低難度)" },
    game1: { title: "關卡一：地圖記憶翻牌", task: "60秒內找出所有成對圖案！錯誤越少越好！(卡牌都是Google地圖常見的圖案)" },
    game2: { title: "關卡二：街景車出任務", task: "收集 15 個「地標」，小心躲避「路障」！(支援鍵盤左右鍵)" },
    game3: { title: "最終關：終極導航迷宮", task: "扮演GOOGLE導航，60秒內引導探險家穿越迷宮！請注意中央有「徘徊的鬼魂」，被抓到就直接失敗！(支援鍵盤左右鍵)" }
  };
                 ...
                 <div className="bg-blue-50 border-b border-blue-200 p-4 flex items-start gap-4">
                    ...
                    <div className="bg-white p-3 rounded-2xl rounded-tl-sm shadow-sm flex-1 text-left border border-gray-200">
                      <p className="text-gray-800 font-bold leading-relaxed text-sm md:text-base">
                        「放學了！我現在在<span className="text-blue-600">『土城國小』</span>，想去<span className="text-red-500">『土城圖書館』</span>借書。大約 500 公尺，今天天氣晴朗，請幫我規劃一條既健康又能活動身體的交通路線吧！」
                      </p>
                    </div>
                 </div>
